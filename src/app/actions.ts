...
export async function createAction(formData: FormData){
    ...
    redirect(`/invoices/${results[0].id}`)//dynamic page, for using need to add redirect
    
}

export async function updateStatusAction(formData: FormData){
    const { userId } = await auth();
    //auth ile sign in yapıp yapmadığına bakıyor


    if( !userId) {
        return ;
        //eğer giriş yapmadıysa return ile atıyor
    }
    ...
    //revalidatePath, belirtilen yolu (/invoice/${id}) yeniden doğrular ve bu sayfanın statik önbelleğini temizler.
    //Kullanıcı fatura sayfasını tekrar ziyaret ettiğinde, en güncel veriler yüklenir.
